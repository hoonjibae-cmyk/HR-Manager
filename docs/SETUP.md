...
```
> 봇을 **세 채널 모두에 초대**해야 메시지를 게시할 수 있습니다 (`/invite @유쌤 알리미`).
...
**① 채널 버튼 메시지 + 고정**
1. `#휴가-신청` 채널에서 `/invite @유쌤 알리미` 로 봇을 초대합니다.
...
2. https://api.slack.com/apps 접속 → 목록에서 **유쌤 알리미** 앱 클릭
...
1. https://api.slack.com/apps → **유쌤 알리미** 앱 클릭
...
1. `#보강-신청` 채널에서 `/invite @유쌤 알리미` 로 봇을 초대합니다.
...
앱 홈(사이드바 → **유쌤 알리미** → 홈)에도 **보강계획 신청** 버튼과 예정된 보강이 함께 나옵니다.
