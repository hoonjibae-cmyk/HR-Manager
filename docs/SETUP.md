...
## 3-A-4. 구글 드라이브 연동 (계약서 서명본 스캔 보관) — 선택

계약 이력 카드의 **＋ 스캔본 첨부** 로 올린 서명·날인본을 구글 드라이브에 보관합니다.
**설정하지 않아도 기능은 그대로 동작합니다** — 그때는 스캔본이 DB 에 저장됩니다.

### ⚠ 먼저 알아야 할 것 — Google Workspace 가 필요합니다

**서비스 계정은 개인 드라이브(내 드라이브)에 파일을 소유할 수 없습니다.** 구글이 2022년경부터
막았습니다. 그래서 올릴 수 있는 곳은 **공유 드라이브(Shared Drive)** 뿐이고,
공유 드라이브는 **Google Workspace 계정에서만** 만들 수 있습니다.

개인 지메일 계정의 폴더를 서비스 계정에 공유해 두면 **조회는 되는데 업로드만 실패합니다**
(`storageQuotaExceeded`). 가장 헷갈리는 실패 모양이라 연결 테스트가 이것을 딱 짚어 줍니다.

> **Workspace 인지 확인하는 법**: 드라이브 왼쪽 메뉴에 **'공유 드라이브'** 항목이 보이면 Workspace 입니다.
> 없으면 개인 계정이므로 이 연동은 건너뛰고 DB 보관을 그대로 쓰세요 (그래도 아무 문제 없습니다).

### 1) Drive API 사용 설정

Google Cloud 콘솔 → **API 및 서비스 → 라이브러리 → Google Drive API → 사용**
(캘린더와 **같은 프로젝트·같은 서비스 계정**을 씁니다. 키를 새로 만들 필요 없습니다.)

### 2) 공유 드라이브에 폴더 만들고 서비스 계정 초대

1. 드라이브 → **공유 드라이브** → 새로 만들기 (예: `유쌤에듀 인사`)
2. 그 안에 폴더 생성 (예: `근로계약서`)
3. 공유 드라이브 → 멤버 관리 → **서비스 계정 이메일** 추가 → 권한 **콘텐츠 관리자** 이상
4. 폴더를 열고 주소창에서 `/folders/` 뒤의 문자열을 복사 → 이것이 폴더 ID

### 3) 환경변수

| 변수 | 값 |
|---|---|
| `GOOGLE_DRIVE_CONTRACT_FOLDER_ID` | 위에서 복사한 폴더 ID |

서비스 계정 키(`GOOGLE_SERVICE_ACCOUNT_JSON`)는 캘린더와 공유하므로 추가 설정이 없습니다.

### 4) 확인

`설정` → **구글 드라이브 연결 테스트**. 토큰 발급 → 폴더 조회 → **시험 업로드·삭제**까지
실제로 해 보고 어디서 막혔는지 단계별로 알려줍니다.
**시험 업로드까지 통과해야** 실제 첨부도 됩니다(조회만 되는 상태가 있기 때문입니다).

### 저장 위치

`지정한 폴더 / 사번 이름 / 2025-10-01 근로계약서.pdf` 형태로 쌓입니다.
직원별 폴더가 자동으로 만들어지고, 파일 이름 앞에 계약 시작일이 붙어 드라이브에서
바로 훑어볼 수 있습니다.

> 드라이브 파일이라도 **앱을 거쳐야만 열립니다**(로그인 필요). 계약서에는 주민등록번호·주소·급여가
> 들어 있어 '링크가 있는 모든 사용자' 공유로 열어 두지 않습니다.

> ⚠ **드라이브에서 파일을 직접 지우거나 옮기면** 앱 목록에는 남아 있는데 열리지 않습니다.
> 정리는 앱의 `✕` 로 하세요(그러면 드라이브 파일도 함께 지워집니다).
