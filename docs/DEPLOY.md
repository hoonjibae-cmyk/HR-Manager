...
- 참고: Vercel **Hobby(무료)** 는 크론이 하루 1회 — 매달 급여 발송엔 충분합니다.

**분 단위로 정확한 시각에 보내려면** (선택): 무료 외부 크론(cron-job.org 등)에서
`https://<도메인>/api/cron?secret=<CRON_SECRET>&strict=1` 을 10분 간격으로 호출하세요.
`strict=1` 이면 앱 `설정`의 **시:분(KST)** 까지 확인해 해당 시각에 발송합니다.
