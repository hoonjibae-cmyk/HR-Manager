...
- **시급제 주휴수당은 세 요건을 모두 갖춘 주에만 붙는다**(`lib/timesheet.ts`, 테스트 있음) —
  ㉠ **1주 소정근로 15시간 이상**(근로기준법 §18③. '미만' 이 제외이므로 15시간 정각은 대상).
  1순위는 **계약 근로시간표**라 실근로가 많다고 주휴시간이 늘지 않는다. 다만 **계약이 15시간 미만이어도
  4주 평균 실근로가 15시간 이상이면 대상에 넣는다**(`eligibleBy: "actual"`) — 계약서에 주2일로 적혀
  있는데 실제로는 매주 3~4일을 나오는 사람이 실제로 있고, 계약 글자만 보고 초단시간으로 분류하면
  주휴가 통째로 사라져 체불이 된다. 이때 주휴시간은 4주 평균 실근로 ÷ 5 로 잡는다.
  평균에서 **기록 없는 주·입사 전 주는 뺀다**(0으로 깔면 평균이 부당하게 내려간다).
  ㉡ **그 주 소정근로일 개근** — **요일이 아니라 '근무일수'로 판정한다**(시행령 §30①).
  계약이 주3일이면 어느 요일이든 3일 이상 나오면 개근. 학원 사정으로 근무 요일이 달마다
  바뀌어(월수 → 월목 → 화수) 요일을 맞추면 실제로 개근한 사람이 결근으로 잡히기 때문.
  연차 사용일은 출근으로 세고, 그 주 공휴일수만큼 채울 일수가 줄며(일요일 공휴일은 제외),
  지각·조퇴는 결근이 아니다.
  ㉢ **그 주(월~일) 내내 근로관계 존속** — 주휴일 전 퇴사·주 중간 입사면 미발생.
  ㉢은 2021.8.4. 행정해석 변경(임금근로시간과-1736)을 따른다: 주휴일까지만 재직하면 8일째
  근로관계가 없어도 발생한다. **'퇴사 주는 무조건 미지급' 은 옛 해석이라 그대로 구현하면 체불**이 된다.
  주휴시간 = min(주 소정근로시간 ÷ 5, 8). 근로시간표가 비면 주 근무일수를 알 수 없어 옛 방식
  (실근로 15시간 초과)으로 갈음하고 `noSchedule` 로 화면에 경고한다.
  달을 걸친 주는 **주휴일(일요일)이 속한 달**에서 한 번만 센다(이중 지급·누락 방지).
- **일별 기록은 DB(`TimesheetDay`)에 남긴다** — 달을 걸친 주를 뒤 달에서 지급하려면 그 주 **앞부분(앞달
  날짜)의 출근 여부**를 알아야 개근을 판정할 수 있는데, 뒤 달 기록표에는 그 며칠이 없다.
  저장을 안 하던 때는 앞달 며칠이 결근으로 잡혀 **앞달에도 뒤 달에도 안 붙어 그 주 주휴가 통째로 사라졌다**.
  업로드할 때마다 `upsert` 로 쌓고(같은 날 다시 올리면 **파일이 이긴다**), 계산 전에 앞뒤 ±14일을 읽어 합친다.
  **한 번도 올라온 적 없는 날은 '결근' 이 아니라 '기록 없음'** 이다(`knownFrom` = 그 직원의 최초 저장일).
  그런 날이 낀 주는 `partial` 로 표시하고 **주휴 판정을 보류**한다 — 돈 문제라 추측해서 주지도 빼지도 않고,
  그 주가 든 앞달 기록표를 한 번 올리면 자동으로 확정된다. `attendedDates` 로 어느 날 나왔는지 화면에 띄운다.
- **계약 근로시간표가 실제와 다르면 화면에 경고**한다(`scheduleMismatch`) — 주휴·연차가 전부 계약을
  기준으로 돌아가므로 계약이 실제보다 적게 적혀 있으면 판정이 통째로 틀어진다. 계약 주 근무일수·주 시간과
  이 달 실제 평균을 나란히 보여주고, 계약이 15시간 미만이면 §18③ 로 주휴·연차가 빠진다는 것과
  직원 상세로 가는 링크를 함께 띄운다. **근본 해결은 계약 근로시간표를 고치는 것**이다.
- **기록표 파싱은 조용히 실패하지 않는다**(`parseHourCell`/`parseDateCell`, 테스트 있음) —
  근무시간 열은 보통 `퇴근−출근` 수식이라 엑셀 시간값(하루=1)이지만 사람이 고친 칸은 그냥 숫자(`5`)나
  문자(`"5:00"`)로 들어온다. 예전엔 그런 칸을 통째로 버려서 **그날이 결근으로 잡히고 그 주 주휴가 사라졌다**.
...
- **시급제 시간은 셋으로 나눠 다룬다**(`MonthlyTimesheetResult`) — `stayHours`(학원 체류, 출퇴근 기록
  ...
  **연차를 쓴 날은 1일 소정근로시간(주 소정 ÷ 주 근무일수)을 채운 것으로 본다** — 연차휴가는
  유급휴가(§60)이므로 기록표가 비어도 그만큼 지급하고 개근에도 출근으로 센다(반차는 0.5일 비례).
  연차 미적용 직원(§18③ 초단시간)은 유급 인정에서 빠지되 개근 판정에는 그대로 센다.
