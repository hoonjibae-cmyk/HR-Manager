...
- **시급제의 소정근로는 계약서가 아니라 '그 달 근로계획표'다.** 계약서의 주당 근로일은 문서화용
  형식일 뿐이고 실제 운영과 어긋난다 — **매월 말에 학원이 다음 달 계획표를 짜서 공지하는 것이
  그 달의 소정근로**이고, 달마다·주마다 바뀐다. 1일 근로시간만 4.5시간 내외로 대체로 일정하다.
  그래서 **계약 주 근무일수로 개근을 따지지 않고, 4주 평균으로 묽히지도 않는다.**
- **시급제 주휴수당은 1주 단위로, 그 주 실근로시간만 보고 판정한다**(`lib/timesheet.ts`, 테스트 있음) —
  ㉠ **그 주 근로 15시간 이상**(§18③. '미만' 이 제외이므로 15시간 정각은 대상).
  한 주라도 15시간을 채우면 **그 주에 한해** 발생하고, 다른 주가 미달이어도 상관없다.
  계획표대로 나온 결과가 실근로이므로 **실근로 충족이 개근을 겸한다** — 별도의 근무일수 개근 판정은
  두지 않는다(요일·일수가 매달 바뀌어 판정 자체가 불가능하다).
  ...
  휴게 차감은 급여 산정과 같은 기준(`breakPaid`)을 쓴다 — **유급이면 체류, 무급이면 순 근로로
  15시간을 재므로 이 설정이 판정을 가를 수 있다**(체류 15.2h = 순 13.2h).
  연차 사용일은 1일 소정근로시간(계약 주 시간 ÷ 주 근무일수)을 채운 것으로 세어 판정에 넣는다.
  달을 걸친 주는 **주휴일(일요일)이 속한 달**에서 한 번만 센다(이중 지급·누락 방지).
  근로시간표는 이제 주휴 판정에 쓰지 않는다 — **연차일의 유급 시간 환산에만** 쓴다.
