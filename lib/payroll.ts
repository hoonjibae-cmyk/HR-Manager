...
export function computePayroll(
  ...
  // --- 퇴직유보금: 인센티브 계약은 **인센티브 원천액**의 1/12(8.3%)를 별도통장 송금(공제) ---
  // 확인서 제6조. 위탁계약은 퇴직급여 대상이 아니므로 유보하지 않는다.
  //
  // **상여금(`bonusP`)은 대상이 아니다** — 이 칸은 비정기 특별상여 자리라 평균임금에
  // 들어가지 않는 경우가 많다. 인센티브는 `인센티브` 칸(학생수 자동산정 + 직접 입력)에 넣는다.
  //
  // 다른 공제와 달리 **10원 절사를 하지 않고 원 단위로 반올림**한다 — 세무사무소에 넘기던
  // 기존 시트가 그렇게 적립해 왔다(200,000원 → 16,667원. 절사하면 16,660원이 되어 어긋난다).
  // 유보금은 법정공제가 아니라 직원 몫으로 떼어 두는 돈이라 깎지 않는 쪽이 맞기도 하다.
  let retentionD = 0;
  if (!contractor && emp.payScheme === "INCENTIVE" && incentiveP > 0) {
    retentionD = round0(incentiveP * RETENTION_RATE);
    notes.push(
      `퇴직유보금: 인센티브 ${incentiveP.toLocaleString()}원 × 1/12(8.3%) = ${retentionD.toLocaleString()}원 (별도통장 송금)`
    );
  }
