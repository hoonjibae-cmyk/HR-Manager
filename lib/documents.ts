...
export function contractHtml(args: {
  ...
  if (!isHourly) {
    // 토요일 당번제는 **교육운영팀의 제도**다 — 교수부 등 다른 부서 계약서에 넣으면
    // 실제로 시키지 않는 토요일 근무·수요일 면제를 근로조건으로 명시하는 셈이 된다.
    // 부서 이름 부분일치는 위 조교팀 조항과 같은 방식(부서 이름을 바꾸면 여기도 고친다).
    if ((e.department ?? "").includes("교육운영")) {
      art3Post.push(
...
export const OVERSEAS_PROCESSORS: Array<{
  ...
  },
  {
    name: "Google LLC",
    country: "미국",
    // 지메일(SMTP)로 급여명세서를 보내므로 **급여 내역이 함께 이전된다** — 캘린더·드라이브와
    // 성질이 다르지만 수탁자가 같아 한 줄로 적는다(고지는 '누구에게' 기준이다).
    purpose:
      "연차·보강 일정 공유(구글 캘린더), 업무 문서 저장·공유(구글 드라이브), 급여명세서 등 문서의 이메일 발송(지메일)",
    items: "성명, 이메일 주소, 휴가·보강 일자, 급여명세서에 기재된 급여·공제 내역, 업무 문서에 포함되는 개인정보",
  },
  {
...
export function consentDeductionHtml(args: { employee: DocEmployee; company: DocCompany; date?: Date }): string {
  ...
  /**
   * **연차 초과사용 정산** — 퇴직 시점까지 발생한 연차보다 많이 쓰고 나가는 경우의 공제 근거.
   *
   * 근로기준법 §43(전액불 원칙) 때문에 임금 공제는 법령·단체협약 또는 **근로자의 자유로운
   * 의사에 의한 명시적 동의**가 있어야 한다. 이 조항이 그 동의다 — 동의 없이 공제하면
   * 초과 지급분을 돌려받을 권리가 있어도 '임금에서 마음대로 깠다' 는 다툼이 된다.
   * 그래서 **계약서가 아니라 이 동의서에 둔다** — 공제 항목을 열거하고 서명받는 문서가
   * 바로 여기다. **위탁계약(프리랜서)에는 적지 않는다** — 연차(§60) 자체가 없다.
   */
  const contractor = isContractorContract(e);
  const leaveRow = contractor
    ? ""
    : `
    <tr><th>연차 정산</th><td>퇴직 시 퇴직일까지 발생한 연차유급휴가보다 초과하여 사용한 연차가 있는 경우,
      그 초과 사용일수에 해당하는 금액(초과일수 × 1일 통상임금)을 마지막 ${insured ? "임금" : "지급액"} 지급 시
      공제·정산함에 동의</td></tr>`;
  // '본 동의서에 명시되지 아니한' — 연차 정산처럼 동의에 근거한 공제가 항목에 늘었으므로,
  // '법령에 근거하지 않은 임의공제 불가' 로 적으면 방금 동의받은 항목과 모순되게 읽힌다.
  const foot = insured
    ? `※ 본 동의는 근로기준법 제43조(임금 전액지급의 원칙)의 예외로서 본인의 자유로운 의사에 따른 것이며, 본 동의서에 명시되지 아니한 임의공제에는 적용되지 아니합니다.`
    : `※ 위 공제는 법령에 따른 원천징수 및 본인이 동의한 정산이며, 본 동의서에 명시되지 아니한 임의공제에는 적용되지 아니합니다.`;
  return `${companyHead(c)}
  ...
  <table class="kv">
    ...
    <tr><th>정산</th><td>과·오납 발생 시 익월 ${insured ? "임금" : "지급액"}에서 정산함에 동의</td></tr>${leaveRow}
  </table>
