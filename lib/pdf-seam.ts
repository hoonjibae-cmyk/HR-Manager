...
/**
 * **인감 이미지를 담는 정사각형 상자의 한 변(mm)** — 본문 날인·간인이 이 값 하나를 함께 쓴다.
 *
 * ⚠ **종이에 찍히는 붉은 원의 지름이 아니다.** 올린 PNG 는 대개 도장 둘레에 투명 여백이
 * 있고, `object-fit:contain` 이 **여백까지 포함해** 상자에 맞추므로 실제 잉크는 그보다 작게
 * 나온다. 유쌤에듀가 올린 인감은 여백이 넉넉해 잉크가 상자의 **6할 남짓**이다.
 *
 * 맞춘 근거(2026-08): 실물 법인인감을 자로 재니 **지름 18mm**. 서명된 계약서에 실물 인감과
 * 시스템 인감이 나란히 찍힌 것을 대조하니 실물이 시스템의 약 **1.4배**였다. 즉 그때
 * 상자 20mm 로 찍힌 잉크는 13mm 남짓이었다는 뜻이고, 18mm 를 내려면 상자가
 * 20 × (18/13) ≈ **28mm** 여야 한다.
 *
 * 그래서 **인감 이미지를 여백 없이 다시 자른 것으로 갈아 끼우면 이 값도 함께 내려야 한다**
 * (여백이 없어지면 상자 = 잉크라 18~20mm 면 된다). 바꾼 뒤에는 한 장 뽑아 자로 재 볼 것.
 *
 * 상업등기규칙 §35② 는 법인 인감을 **한 변 1cm 초과 3cm 이내**로 정한다 — 그건 잉크 기준이라
 * 상자가 30mm 를 넘어도 규칙 위반은 아니지만, 넘겨야 할 만큼이면 이미지를 다시 자르는 게 맞다.
 *
 * ⚠ 여기만 고치면 본문·간인·축소본이 함께 따라온다. **CSS 에 숫자를 다시 적지 말 것.**
 */
export const STAMP_SIZE_MM = 28;
/** 간인 도장의 지름(mm) — 본문 인감과 같다 */
export const SEAM_SIZE_MM = STAMP_SIZE_MM;
/** `.compact` 조판(서약서 등)에서 쓰는 지름 — 본문 대비 85% */
export const STAMP_COMPACT_MM = Math.round(STAMP_SIZE_MM * 0.85);
/**
