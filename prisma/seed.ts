...
async function main() {
  ...
  // 4) 공휴일 (2025~2027)
  //
  // ⚠ **여기를 손으로 채우는 것은 임시방편이다.** 음력 명절·대체공휴일·임시공휴일은 규칙만으로
  // 나오지 않아서, 해가 바뀌면 이 표는 반드시 뒤처진다(실제로 2026년 한글날·부처님오신날·
  // 대체공휴일 넷이 통째로 빠져 있었다). 표가 빠지면 그날 근무가 평일로 잡혀 휴일근로
  // 가산(×1.5)이 조용히 안 붙는다.
  //
  // 진짜 출처는 한국천문연구원 「특일 정보」다 — `npm run db:holidays-sync`(또는 설정 화면의
  // *공휴일 받아오기*, 크론). 이 표는 인증키가 없는 개발 환경에서 화면이 비지 않게 두는
  // **초기값**이다. 동기화는 넣기만 하고 지우지 않으므로 나중에 받아도 부딪히지 않는다.
  const holidays = BUILTIN_HOLIDAYS;
  for (const { date: d, name } of holidays) {
